...
# feature scaling
train$DISCOUNT_PRICE <- train$DISCOUNT_PRICE / max(train$DISCOUNT_PRICE)

train$VALIDPERIOD[is.na(train$VALIDPERIOD)] <- -1
train$VALIDPERIOD <- train$VALIDPERIOD + 1
train$VALIDPERIOD <- train$VALIDPERIOD / max(train$VALIDPERIOD)

train$DISPPERIOD[is.na(train$DISPPERIOD)] <- -1
train$DISPPERIOD <- train$DISPPERIOD + 1
train$DISPPERIOD <- train$DISPPERIOD / max(train$DISPPERIOD)
...
user_profile <- aggregate(.~USER_ID_hash, data=rbind(user_profile,train[,-1]), FUN=mean)
...
#     user_profile <- aggregate(.~USER_ID_hash, data=rbind(user_profile,trainv[,-1]), FUN=mean)    
# }
...
weightm <- c(2.00, 0.00, 1.25, 1.00, 4.50, 0.625, 0.35) # males weights
weightf <- c(1.75, 0.00, 1.50, 2.50, 6.50, 0.625, 0.25) # female weights

Wm <- as.matrix(Diagonal(x=c(rep(weightm[1],13), rep(weightm[2],1), rep(weightm[3],1), rep(weightm[4],9), rep(weightm[5],55), rep(weightm[6],1), rep(weightm[7], 1))))
Wf <- as.matrix(Diagonal(x=c(rep(weightf[1],13), rep(weightf[2],1), rep(weightf[3],1), rep(weightf[4],9), rep(weightf[5],55), rep(weightf[6],1), rep(weightf[7], 1))))
